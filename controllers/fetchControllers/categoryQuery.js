...
const categoryQuery = {
  ...
  getCategoryWithDetails: async (req, res) => {
    ...
    getCategoriesWithAvailableBooks: async (req, res) => {
        try {
            const categories = await Category.findAll({
                attributes: ['name', [Sequelize.fn('COUNT', Sequelize.col('books.id')), 'bookCount']],
                include: [{
                    model: Book,
                    attributes: [],
                    where: {
  ...
  // 3. Search books in a specific category by title or author
   searchBooksInCategory: async (req, res) => {
    try {
        ...
        try {
            const categories = await Category.findAll({
                ...
                attributes: [
                    'id',
                    'name',
                    [sequelize.fn('COUNT', sequelize.col(`books->borrowings.id`)), 'borrowCount']
                ],
                group: ['Category.id'],
